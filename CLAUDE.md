...
| JOB保存数 | 5 | 無制限 | 無制限 | 保存可能なJOB数 |
| クラウド同期 | — | ○ | ○ | JOBのクラウド同期 |
| AIコードエディター | — | 月200回 | 無制限 | AIによるPythonコード生成・編集 |
| オーケストレーター | — | — | ○ | Agent集中管理・JOB配信・実行監視 |
| Agent管理 | — | — | 無制限 | 管理可能なInsight Business Suite端末数 |
...
**プラン別制限（INMV固有）:** FREE: 最大200MB / 1080p、BIZ+: 無制限 / 4K
...
| キャラクタープロンプト | 5個 | 無制限 | 無制限 | 保存可能なプロンプト数 |
| 高解像度出力 | — | ○ | ○ | 高解像度画像の生成 |
| クラウド同期 | — | ○ | ○ | プロンプト・設定の同期 |
...
| AIアシスタント | — | 月200回 | 無制限 | AIによるスライドテキストの校正・改善提案 |
| AIコードエディター | — | 月200回 | 無制限 | AIによるPython自動処理 |
| 参考資料 | ○ | ○ | ○ | 参考資料の添付・AIコンテキスト活用 |
| ドキュメント評価 | — | 月200回 | 無制限 | AIによる多角的評価・スコアリング |
...
| AIアシスタント | — | 月200回 | 無制限 | AIチャットによるExcel操作支援 |
| AIコードエディター | — | 月200回 | 無制限 | AIによるPython自動処理 |
| 参考資料 | ○ | ○ | ○ | 参考資料の添付・AIコンテキスト活用 |
| ドキュメント評価 | — | 月200回 | 無制限 | AIによる多角的評価・スコアリング |
| 音声入力 | ○ | ○ | ○ | 音声認識によるハンズフリー入力 |
...
| バッチ処理 | — | ○ | ○ | 複数ドキュメントの一括処理 |
| マクロ実行 | — | ○ | ○ | VBAマクロの実行・変換 |
| AIアシスタント | — | 月200回 | 無制限 | AIによるドキュメントの校正・要約・構成提案 |
| AIコードエディター | — | 月200回 | 無制限 | AIによるPython自動処理 |
| 参考資料 | ○ | ○ | ○ | 参考資料の添付・AIコンテキスト活用 |
| ドキュメント評価 | — | 月200回 | 無制限 | AIによる多角的評価・スコアリング |
| 音声入力 | ○ | ○ | ○ | 音声認識によるハンズフリー入力 |
...
| コード実行 | ○ | ○ | ○ | Pythonコードの実行 |
| プリセット利用 | ○ | ○ | ○ | 定義済みスクリプトテンプレート |
| スクリプト保存数 | 5 | 無制限 | 無制限 | 保存可能なスクリプト数 |
| クラウド同期 | — | ○ | ○ | スクリプトのクラウド同期 |
| AIコードエディター | — | 月200回 | 無制限 | AIによるPythonコード生成・編集・デバッグ |
...
| AIアシスタント | — | 月200回 | 無制限 | 自然言語操作支援（「A2に1万円入れて」等） |
| 音声入力 | ○ | ○ | ○ | 音声認識によるハンズフリー入力 |
| 読み上げ | ○ | ○ | ○ | メール・文書の音声読み上げ |
...
| プラン | 説明 | 対象 | 有効期限 |
|-------|------|------|---------|
| FREE | 基本機能のみ（ライセンス不要） | 評価・個人利用 | 無期限 |
| TRIAL | 全機能利用可能（評価用・Premium AI ティア） | 評価企業 | 1ヶ月（自動発行） |
| BIZ | 全機能（1PC・AI月200回・Standard AI ティア） | 法人・個人利用 | 365日 |
| ENT | 全機能 + コラボレーション（AI無制限・Premium AI ティア） | 法人・チーム | 要相談 |

### Insight Performance Management (IOSH) 機能マトリクス

> **FREE = 基本機能（AI なし）**、**BIZ = 全機能+AI月200回（1人利用）**、**ENT = BIZ + コラボレーション + AI無制限**

| 機能 | TRIAL | FREE | BIZ | ENT |
|------|:-----:|:----:|:---:|:---:|
| Excel読み込み・編集 | ✅ | ✅ | ✅ | ✅ |
| バージョン管理 | ✅ | ✅ | ✅ | ✅ |
| 差分比較 | ✅ | ✅ | ✅ | ✅ |
...
| ライセンス制御 | FREE: 利用不可 / TRIAL: 無制限 / BIZ: 月200回 / ENT: 無制限 |
...
|--------|----------------|---------------|---------|
| Standard | 最新 Sonnet | Haiku + Sonnet 系 | BIZ プラン、Standard アドオン |
...
// ライセンスチェック
canUseAiAssistant('BIZ');   // true（月200回）
...
// → "AIアシスタント（スタンダード（Sonnet 4.6））— 残り 190回"
```
...
├── ai_memory_deep/              # AI ディープストレージ（BIZ+）
│   ├── glossary.json
│   ├── people.json
│   ├── projects.json
│   └── context.json
...
// プラン別容量制限チェック
checkProjectFileLimits('FREE', { historyVersions: 25 });
...
canAddAgent('ENT', 50);     // true（ENT は無制限）
...
canEnableModule('IOSH', 'bot_agent', 'BIZ', ['python_runtime']);  // { allowed: true }
```
...
| パターン | 対象ユーザー | プラン | 機能 |
|---------|------------|--------|------|
| **個人 AI 利用** | 一般ユーザー | BIZ | AI チャット + 全機能 |
| **市民開発** | パワーユーザー | BIZ | Python + AI エディター + ローカルワークフロー |
| **リモート RPA** | BPO / IT 部門 | ENT (INBT) | Orchestrator + Agent + スケジューラー |

### ローカルワークフロー（BIZ Insight Business Suite）

BIZ の Insight Business Suite ユーザーは Orchestrator なしで、ローカル PC 上の簡易自動化が可能。

```typescript
import { canEnableModule } from '@/insight-common/config/addon-modules';

// BIZ ユーザーはローカルワークフローを有効化可能
canEnableModule('IOSH', 'local_workflow', 'BIZ', ['python_runtime', 'python_scripts']);
// { allowed: true }

// FREE ユーザーは不可
canEnableModule('IOSH', 'local_workflow', 'FREE', ['python_runtime', 'python_scripts']);
// { allowed: false, reasonJa: 'ローカルワークフローには TRIAL/BIZ/ENT プランが必要です' }
```
...
- [ ] **AI アシスタント**: ライセンスゲート（FREE: 利用不可 / TRIAL: 無制限 / BIZ: 月200回 / ENT: 無制限）が実装されている
...
- [ ] **ローカルワークフロー**: BIZ Insight Business Suite でローカル連続処理が動作する（BIZ+ のみ）
