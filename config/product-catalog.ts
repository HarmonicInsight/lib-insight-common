...
export const PRODUCT_CATALOG: CatalogEntry[] = [
...

  {
    ...
    description: {
      ...
      ja: '今お使いのExcelファイルをこのツールで開くだけで、バージョン管理、セル単位の変更履歴管理、掲示板機能、AIチャットが実現できます。搭載のAIアシスタントが数値や計算式をチェック・修正し、複雑な財務モデルの正確性を確保します。クラウドのような環境は不要で、共有サーバーに置いておけば1つのファイルを複数の人とコラボレーションし、誰がいつ何を変更したかを確認しながら作業を進められます。',
  ...
  },
  {
    ...
    description: {
      ...
      ja: 'Wordファイルは1つだけ。履歴は全部残る。参照資料としてExcel・Wordファイルを登録でき、AIアシスタントがその内容を基にアドバイスします。「この文書を要約して」「第3章に何を書けばいい？」など、登録した参考資料に基づいたコンテキスト対応の回答が得られます。各種書類作成の効率を根本的に変えるツールです。',
    ...
    features: {
      ...
      ja: [
        'AIアシスタント — 登録した参考資料に基づくコンテキスト対応のアドバイス',
        'Wordファイルの全履歴管理',
        '参考資料の登録（Excel / Word）',
        'Word連携編集（Wordで開いて同期）',
        'エクスポート機能',
