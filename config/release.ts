...
export const RELEASE_CONFIGS: Record<ReleaseTargetCode, ReleaseConfig> = {
  ...
  INCA: {
    ...
    notes: 'Tauri デスクトップアプリ。インストーラーで直接配信。',
  },
  ...
  INSS: {
    ...
    notes: 'WPF デスクトップアプリ。インストーラーで直接配信。独自拡張子 .inss。',
  },
  ...
  IOSH: {
    ...
    requiredLocales: [],
    buildCommands: ['dotnet build -c Release', 'dotnet publish -c Release'],
    notes: 'WPF デスクトップアプリ。インストーラーで直接配信。独自拡張子 .iosh。',
  },
  ...
  IOSD: {
    ...
    requiredLocales: [],
    buildCommands: ['dotnet build -c Release', 'dotnet publish -c Release'],
    notes: 'WPF デスクトップアプリ。インストーラーで直接配信。独自拡張子 .iosd。',
  },
...
export const CAMERA_STORE_METADATA: Record<string, StoreMetadata> = {
  'ja-JP': {
    ...
    shortDescription: 'シンプルで綺麗に撮れるカメラ。常時ライト・ワンタッチ操作・OEM画質自動適用。',  // 37文字（上限80）
    fullDescription:
      ...
      '・ワンタッチ操作 — フォーカス、撮影、録画がすべてワンタッチで完結\n\n' +
      '■ 撮影機能\n' +
      '・写真撮影（JPEG 最高画質）\n' +
      '・動画録画（MP4 最高画質・音声付き）\n' +
