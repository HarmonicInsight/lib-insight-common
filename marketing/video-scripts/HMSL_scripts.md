...
| 旧 HMSL 機能 | INSS での対応 |
|-------------|-------------|
| テンプレート生成 | INSS テンプレート・バッチ処理に統合 |
| PDF化 | INSS バッチ処理に統合 |
| スライド抽出 | INSS テキスト・画像一括抽出に統合 |
| バッチ処理 | INSS バッチ処理に統合 |
| 読み込み・解析 | INSS JSON入出力・ファイル比較に統合 |

---

## 旧スクリプト（参考用・アーカイブ）

以下は統合前のHarmonicSlide用スクリプトです。新規制作には使用しないでください。
...
| 8-15秒 | 「~~HarmonicSlide~~ → **Insight Deck Quality Gate** BIZのバッチ処理なら、全ファイルを一括更新できるのだ！」 | バッチ処理設定画面のスクショ |
| 15-22秒 | 「ロゴ差し替え・テキスト更新・スライド追加、全部自動なのだ」 | 処理中 → 完了のスクショ |
| 22-27秒 | 「30ファイルの更新が10分で終わるのだ！」 | ビフォー1日 → アフター10分 |
| 27-30秒 | 「Insight Deck Quality Gate、無料トライアルはこちらなのだ！」 | ロゴ + URL + CTA |
