...
| Q6 | 空の catch ブロックがない | ✅ | `catch { }` や `catch (Exception) { }` で例外を握りつぶしていない |
| Q7 | バージョン番号が全箇所で一致 | ✅ | .csproj / XAML / ViewModel / インストーラー等での重複を排除 |
...
| L6 | XAML 内に日本語ハードコードテキストがない（WPF） | ✅ | `Text="日本語"` / `Header="日本語"` / `Label="日本語"` 等を検出 |
| L7 | C# コード内のユーザー向けメッセージがローカライズ済み | ✅ | エラーメッセージ・ダイアログテキスト等が `LanguageManager` / リソース経由 |
...
| S5 | ライセンス検証の秘密鍵がソースにハードコードされていない | ✅ | `SECRET_KEY` / `HMAC` キー等。逆コンパイルでキー偽造可能 |
| S6 | API キー（Claude 等）が暗号化保存されている | ⚠️ | DPAPI / Windows Credential Manager / Keychain 経由。平文保存禁止 |
| S7 | ライセンスデータが保護されている | ⚠️ | %APPDATA% の license.json が平文でないこと。DPAPI 推奨 |
| S8 | .gitignore に機密ファイルパターンが含まれている | ✅ | `.env*`, `credentials*`, `*.secrets`, `appsettings.*.json` |
...
| W4 | バージョン番号が .csproj / XAML / C# コードで一致 | ✅ | 複数箇所のハードコードを排除 |
| W5 | Copyright 年が最新（現在年を含む） | ✅ | .csproj の `<Copyright>` |
...
| WS4 | ライセンス秘密鍵がハードコードされていない | ✅ | `SECRET_KEY` / HMAC キーの直書き禁止 |
| WS5 | Claude API キーが DPAPI / Credential Manager で保護 | ⚠️ | メモリ上・ディスク上で平文禁止 |
| WS6 | license.json が暗号化されている | ⚠️ | %APPDATA% の平文 JSON 禁止。DPAPI 推奨 |
...
| # | チェック項目 | 自動 | 説明 |
|---|------------|:----:|------|
| WA1 | 主要 UI コントロールに `AutomationProperties.Name` が設定 | ✅ | スクリーンリーダー対応。ボタン・入力欄・メニュー項目 |
| WA2 | キーボードナビゲーションが機能する | ⚠️ | Tab/Enter/Escape で基本操作可能 |

### 5.5 コード品質（WPF 固有）

| # | チェック項目 | 自動 | 説明 |
|---|------------|:----:|------|
| WQ1 | 空の catch ブロックがない | ✅ | 例外を握りつぶさない |
| WQ2 | IDisposable 実装クラスの Dispose でイベント購読解除 | ⚠️ | メモリリーク防止 |
| WQ3 | ToolTip が操作ボタンに設定されている | ⚠️ | 最小化/最大化/閉じるボタン含む |
| WQ4 | ハイパーリンク URL の `javascript:` スキームが検証されている | ✅ | 許可スキームのホワイトリスト（http/https/mailto） |
| WQ5 | `UnobservedTaskException` ハンドラが登録されている | ✅ | App.xaml.cs でログ出力付きハンドラ。SetObserved のみは不可 |
...
| AI2 | BYOK（クライアント選択）モデル設定が実装 | ✅ | モデルティア制限なし |
| AI3 | AI 機能が BYOK 対応（使用回数制限なし） | ✅ | クレジット管理不要 |
